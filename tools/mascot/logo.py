...
PROMPT = (
    ...
    "script, as if handwritten with a BROAD-nib fountain pen - fluid connected "
    "letterforms with BOLD, heavy ink strokes (thick downstrokes, still graceful "
    "thin upstrokes), GENEROUS letter spacing so the script breathes, in a warm "
    "dark brown ink that matches the mascot's brown, with a subtle gold accent "
    "allowed on a flourish. Spell it exactly: 'Ram Plan' (capital R, capital P, "
    "two words).\n"
    "Layout: single horizontal line, head vertically centered against the text, "
    "with a WIDE clear gap between the head and the first letter - the text sits "
    "distinctly apart from the head, never crowding it. Everything inside the "
    "central 80% of the canvas with clear margin on all sides.\n"
